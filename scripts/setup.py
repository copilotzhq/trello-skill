"""Idempotent, configuration-driven three-board setup for Trello.
...
This board holds continuous initiatives or products.
An initiative stays alive while focus and milestones shift.
...
- Paused: Intentionally not receiving work; record resume condition.
- Retired: Concluded or replaced; kept for reference.

Record health in the card description, not as duplicate status lists.
"""
...
- In Progress: Actively being pursued by contributing tasks.
- In Review: Target state reached, verification/evidence being evaluated.
- Achieved: Verified and accepted.
- Paused: Deferred; record reason.

Link contributing tasks from the execution board; do not duplicate task status here.
"""
...
Use the native To-dos checklist on the card for editable execution steps.
"""
...
BOARD_SPECS = {
    ...
    'milestones': {
        ...
        'checklist_items': ['Define acceptance criteria before committing']
    },
    'tasks': {
...
def validate_config(cfg):
    ...
    # Check for duplicate board target IDs or names across roles
    seen_ids = set()
    seen_names = set()
    for role in ['projects', 'milestones', 'tasks']:
        if True:
...
def build_plan(client, cfg, workspace_id):
    validate_config(cfg)
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Automated 3-board setup for Trello.')
    ...
    try:
        ...
        actions, board_targets = build_plan(client, cfg, workspace_id)
        ...
        remaining, _ = build_plan(client, cfg, workspace_id)
        if remaining:
